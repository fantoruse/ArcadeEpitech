...
void Core::OpenFirstLibs(std::string &arg, LoadLib &ldb)
{
    try {
        ...
        _loadLibs.push_back(
            std::pair<std::string, IDisplayModule *>(arg, libs));
        getTypes(arg, ldb);
    } catch (const std::runtime_error &e) {
        std::cerr << e.what() << std::endl;
        throw std::exception();
...
void Core::OpenLibsInLibs(const LoadLib &ldb, const std::string &name)
{
    try {
        auto libs = ldb.loadingLib<IDisplayModule * (void)>("createGraphLib")();
        _loadLibs.push_back(
            std::pair<std::string, IDisplayModule *>(name, libs));
    } catch (const std::runtime_error &e) {
        std::cerr << e.what() << std::endl;
    }
}
...
void Core::gameLoop()
{
    ...
    while (1) {
        ...
        if (event == arcade::CLOSE ||
            _loadGames[y % _loadGames.size()].second->isLost()) {
            _loadLibs[a % _loadLibs.size()].second->destroy();
            break;
        }
        this->game(_loadGames[y % _loadGames.size()].second, event,
            _loadLibs[a % _loadLibs.size()].second);
