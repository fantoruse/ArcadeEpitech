...
arcade::Pacman::Pacman() : arcade::AGame("pacman"), _score(0), _name("Pacman"),
    _map(), _enemies(), _objects(), _started(false)
{
    int n = 0;
...
const std::vector<std::shared_ptr<arcade::IObject>> arcade::Pacman::play(
    arcade::events_e ev
)
{
    if (_objects.empty())
    ...
    temp.push_back(init_object(true, "player", createDrawableVector("player"),
        std::make_pair(_playerPosition.first, _playerPosition.second)));
    for (std::size_t i = 0; i < _enemies.size(); ++i)
        if (_stats == true) {
            temp.push_back(
                init_object(true, "stat", createDrawableVector("stat"),
                    std::make_pair(_enemies[i].first, _enemies[i].second)));
        } else {
            temp.push_back(
                init_object(true, "enemie", createDrawableVector("enemie"),
                    std::make_pair(_enemies[i].first, _enemies[i].second)));
        }
...
static const std::map<arcade::events_e, std::pair<float, float>> DIRECTIONS = {
    {arcade::DOWN, std::make_pair(1, 0)}, {arcade::LEFT, std::make_pair(0, -1)},
    {arcade::RIGHT, std::make_pair(0, 1)}, {arcade::UP, std::make_pair(-1, 0)}};

static const std::map<int, std::pair<float, float>> DIRECTIONSENEM = {
    {0, std::make_pair(1, 0)}, {1, std::make_pair(0, -1)},
    {2, std::make_pair(0, 1)}, {3, std::make_pair(-1, 0)}};
...
{
    if (is_static)
        return std::make_shared<arcade::StaticObject>(
            StaticObject(name, drawables, pos));
    else
        return std::make_shared<arcade::MovableObject>(
            MovableObject(name, drawables, pos));
}
...
void arcade::Pacman::init_all_object()
{
    for (std::size_t y = 0; y < _map.size(); y++) {
        for (std::size_t x = 0; _map[y][x] != '\0'; x++) {
            if (_map[y][x] == '#') {
                _objects.push_back(
                    init_object(true, "Wall", createDrawableVector("Wall"),
                        std::make_pair(y, x)));
            }
            if (_map[y][x] == ' ') {
                _comsomable.push_back(
                    init_object(true, "ball", createDrawableVector("ball"),
                        std::make_pair(y, x)));
            }
            if (_map[y][x] == '0') {
...
) const
{
    ...
    if (DRAWABLE_LIST.find(name) == DRAWABLE_LIST.end())
        return dest;
    dest.push_back(
        std::make_shared<arcade::Drawable>(DRAWABLE_LIST.at(name)[0]));
    dest.push_back(
        std::make_shared<arcade::Drawable>(DRAWABLE_LIST.at(name)[1]));
    dest.push_back(
        std::make_shared<arcade::Drawable>(DRAWABLE_LIST.at(name)[2]));
    return dest;
}
...
std::shared_ptr<arcade::IObject> arcade::Pacman::updateScore()
{
    std::vector<std::shared_ptr<arcade::IDrawable>> dest;
    dest.push_back(
        std::make_shared<arcade::Drawable>("apple.png", 10, arcade::NO_TYPE,
            arcade::NONE));
    dest.push_back(
        std::make_shared<arcade::Drawable>("Rect", 50, arcade::NO_TYPE,
            arcade::RED));
    dest.push_back(
        std::make_shared<arcade::Drawable>("Score: " + std::to_string(_score),
            2, arcade::TEXT, arcade::CYAN));

    return std::make_shared<arcade::StaticObject>(
        StaticObject("Score", dest, std::make_pair(10, 30)));
}
...
void arcade::Pacman::enemiesMoves()
{
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(end1 - start1) >=
        std::chrono::milliseconds(600 + _slow)) {
        while (n != _enemiesMov.size()) {
            dir = std::rand() % 4;
            _enemiesMov[n] = DIRECTIONSENEM.at(dir);
            while (!notAWall) {
                if (_map[_enemies[n].first + _enemiesMov[n].first][
        ...
        std::chrono::duration_cast<std::chrono::milliseconds>(end2 - start2) >=
            std::chrono::milliseconds(750 + _slow)) {
        while (n != _enemiesMov.size()) {
            if (_map[_enemies[n].first + _enemiesMov[n].first][
                _enemies[n].second + _enemiesMov[n].second] != '#') {
...
void arcade::Pacman::enemiesGetOut()
{
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(end - start) >=
        ...
        while (n != _enemies.size()) {
            if (_enemies[n].first == 9 && _enemies[n].second >= 9 &&
                _enemies[n].second <= 11) {
                _enemies[n].first = 7;
